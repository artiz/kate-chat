...
| `DOCLING_LAYOUT_ONNX` / `DOCLING_TABLEFORMER_{ENCODER,DECODER,BBOX}` | optional explicit model overrides. The image does not pin them: models resolve via `/usr/local/models` and the pipeline picks the right variant at runtime — INT8 layout on CPU (~2.4× faster at unchanged conformance), fp32 on GPU (the int8 QDQ graphs are CPU-calibrated), and the hoisted-KV TableFormer decoder (`decoder_kv.onnx`) over the legacy graphs |
| `DOCLING_OCR_{REC_ONNX,DICT}` | OCR recognition model + character dictionary. **The image defaults to the English PP-OCRv3 model** (`/opt/models/ocr_rec_en.onnx` + `en_dict.txt`): docling.rs's bundled `ocr_rec.onnx`/`ppocr_keys_v1.txt` is the *Chinese* PP-OCRv3 variant, which under-predicts spaces on Latin text (`writtenonJava`). Point both variables at `/opt/models/ocr_rec.onnx` + `ppocr_keys_v1.txt` to switch back to the multilingual model for CJK-heavy corpora. For local (non-Docker) runs fetch the English pair next to the bundled models: `en_PP-OCRv3_rec_infer.onnx` from `SWHL/RapidOCR` (HF, `PP-OCRv3/`) and `en_dict.txt` from PaddleOCR |
