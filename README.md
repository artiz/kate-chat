...
- LLM tools (Web Search, Code Interpreter, Reasoning) support, custom WebSearch tool implemented using Yandex Search API (optional `YANDEX_SEARCH_SMART_SNIPPETS` mode takes the query-relevant excerpt straight from the Search API instead of downloading the pages; billed extra and served for the Russian index only, so it applies to searches in Russian)
...
- MCP tool call approval: a server marked "Ask before calling tools" makes each call wait in the chat for the user's Approve/Deny, see [docs/notifications.md](docs/notifications.md)
- Browser notifications when an answer finishes or fails, or a tool call waits for approval, while the user is in another chat, tab or window
