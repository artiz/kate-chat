...
- Skills: the model writes a program that builds a PPTX, PDF or XLSX file, the browser runs it in a sandbox (TypeScript or Python via Pyodide) and the file is attached to the answer and listed in the Library; skills are read-only files in `api/resources/skills`, see [docs/skills.md](docs/skills.md)
